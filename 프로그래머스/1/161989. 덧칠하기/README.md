# 메모
아예 생각을 잘못 하여 시도했던 문제. 모든 칸(n)에 대해서 반복문을 돌리거나 간격의 차를 이용해서 풀어보려고 했지만 중간의 공백에 대해서 해결하지 못함.
section안에 들어있는 칠해야되는 값들로만 해결을 했어야 됨. 순회대상이 전체가 아니라 칠해야되는 벽들로만 보면 됨 현재칸이 칠해진 칸이면 패스 아니면 +1.
이문제는 구간 길이나누기 문제가 아니다!!
